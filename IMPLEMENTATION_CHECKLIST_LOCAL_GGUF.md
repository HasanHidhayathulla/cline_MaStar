...
- [x] 114. Read `catalog-live.ts` model-resolution path end-to-end (SPIKE) — **DONE.** Host path is `ProviderCatalog.resolveModels` (`apps/vscode/src/sdk/model-catalog/catalog.ts`) → `resolveSdkModels` → `@cline/core` `resolveProviderConfig`, which merges `knownModels` from the provider manifest + models.dev live catalog + private/public sources. Two findings drove the design: (a) `catalog-live.ts` normalizes models.dev payloads only — it has no per-provider hook a local file could plug into; (b) `resolveProviderConfig` swallows its own resolution errors unless `failOnError`, so the host cannot learn "file missing" from the SDK. Resolution *and* failure reporting therefore both belong to the host, with the file parsing itself in the SDK (`@cline/llms`).
- [x] 115. Add `local-gguf` branch to `resolveModels` — **DONE, data-shape based rather than a provider-id branch.** New `apps/vscode/src/sdk/model-catalog/local-model-file.ts` owns the rule: a provider is file-backed when its effective config carries `modelPath` **and** the SDK declares no `modelsSourceUrl` for it. `resolveSdkModels` delegates to it and returns a `host-adapter`-sourced record; every other provider keeps the SDK path untouched. No `providerId === "local-gguf"` string appears anywhere (per `.clinerules/general.md`).
- [x] 116. Implement `resolveLocalGGUFModels(providerId, options)` returning `Record<string, ModelInfo>` — **DONE** in new `sdk/packages/llms/src/providers/gguf-models.ts`; options are `{ modelPath, forceRefresh, contextWindow, maxTokens }`. Exported from the node entry (`providers.ts` → `index.ts`) alongside the GGUF parser + inference surface; deliberately absent from `index.browser.ts` (needs `node:fs`). Returns `{}` when no `modelPath` is configured.
- [x] 117. Build model record from parsed GGUF metadata (single model expected) — **DONE.** Exactly one entry keyed by exported `LOCAL_GGUF_MODEL_ID = "local-model"`, matching the builtin spec's `defaultModelId` and `GGUFInferenceHandler.getModel()`.
- [x] 118. Set `ModelInfo.name` from `general.name` (fallback: filename stem) — **DONE.** `GGUFMetadata` gained a `name` field (`general.name` verbatim; the parser previously folded it into `modelType`), so the chain is `general.name` → `general.type` → filename stem (new `filenameStem()` in the parser) → `"local-model"`. The literal `"unknown"` the parser uses for absent structural fields is filtered out so it cannot shadow the filename fallback (caught by test).
- [x] 119. Set `ModelInfo.description` from `general.description` when present — **DONE.** `GGUFMetadata.description` is omitted from the model record when empty.
- [x] 120. Set `ModelInfo.contextWindow` from `*.context_length` (> 0) — **DONE**, with a deliberate clamp: the file's `*.context_length` is the ceiling and a user-configured window (`providers.json` `contextWindow`) narrows it via `Math.min`. Rationale: `ModelInfo.contextWindow` drives the agent's token budget while `llama-server` is started with `-c <configured window>`, so reporting more than the server was given would cause context overflows (this is also what Phase 22 task 420 asks for). Resolution order: `min(file, configured)` → file → configured → the provider's own declared `contextWindow` config-field default → `DEFAULT_LOCAL_GGUF_CONTEXT_WINDOW`.
- [x] 121. Set `ModelInfo.maxTokens` default `4096` (OPT) — **DONE** (`DEFAULT_LOCAL_GGUF_MAX_TOKENS`, overridable via `options.maxTokens`).
- [x] 122. Set `ModelInfo.supportsTools` = true (llama.cpp supports `chat-completions` with tools) — **DONE as `capabilities: ["tools"]`.** The SDK `ModelInfo` shape has no boolean `supportsTools` (same deviation Phase 5 recorded for `getModel()`); the capability list is what the SDK and the host's `adaptSdkModelInfo` both understand.
- [x] 123. Set `ModelInfo.supportsImages` from multimodal architecture keys (OPT) — **DONE (best-effort).** New `detectVisionEncoder(raw)` in the parser: `clip.has_vision_encoder` / `clip.projector_type`, or a vision architecture name (`llava|vision|mllama|internvl|minicpmv|pixtral|moondream|〈qwen|gemma〉Nvl`). Surfaced as `capabilities: ["tools","images"]` + `modalities: { input: ["text","image"], output: ["text"] }`, which the host adapter maps back to `supportsImages`.
- [x] 124. Fallback: return a single `"local-model"` entry when metadata parse fails — **DONE, with the boundary pinned down.** "Metadata parse fails" = the GGUF header is valid but carries no usable `general.*` keys; every field then falls back individually, so the picker always gets one selectable entry (a header with zero KV entries yields the filename stem). A file that cannot be *read* (missing/wrong extension) or is not a usable GGUF (bad magic / truncated / unsupported version) is a failure, not a fallback — those throw a typed `GGUFParseError` (tasks 125/130/131).
- [x] 125. Ensure `resolveModels` never **throws** for missing file → return `ok: false` + message — **DONE.** `resolveLocalGGUFModels` throws a typed `GGUFParseError`; `catalog.ts` `resolveSdkModels` lets it propagate and the existing `resolveModels` catch funnel turns it into `{ ok: false, error }`. `toCatalogError` gained a `GGUFParseError` branch delegating to `toLocalModelFileCatalogError`, which maps codes to a structured `CatalogError`: `ENOENT`/`NOT_A_GGUF_PATH` → `kind: "config"`, `NOT_GGUF`/`TRUNCATED`/`UNSUPPORTED_VERSION` → `kind: "shape"`, each keeping `code`. The webview therefore gets an actionable state instead of an empty list.
- [x] 126. Respect `forceRefresh` option (re-parse the file) — **DONE, through both caches.** `resolveModels(providerId, { forceRefresh })` now passes the flag into `resolveSdkModels`, which forwards it to the SDK resolver; the SDK helper's own path+mtime cache is bypassed by the same flag. Covered by a test that rewrites the file with its original mtime restored: the cached call reports the old name, `forceRefresh` reports the new one.
- [x] 127. Cache parsed metadata per file path + mtime (avoid re-read on every call) — **DONE.** Module-level `METADATA_CACHE` keyed by absolute path with `{ mtimeMs, size }`; a hit costs one `stat`. `clearLocalGGUFModelCache()` drops it, and `_testing.cacheSize()` / `_testing.getCachedMetadata()` expose it for assertions (matching the host catalog's `_testing` convention).
- [x] 128. Add `resolveLocalGGUFModels` unit test with a temp `.gguf` fixture — **DONE.** `sdk/packages/llms/src/providers/gguf-models.test.ts` writes real GGUF headers into `mkdtempSync` fixtures (encoder mirrors `gguf-parser.test.ts`). Asserts the single `local-model` entry, `name`/`description`/`contextWindow`/`maxTokens`/`capabilities`, the vision→images promotion, and the sparse-header filename fallback.
- [x] 129. Test cache hit/miss behavior — **DONE.** The "serves a cache hit while mtime and size are unchanged, and re-reads on forceRefresh" test rewrites the fixture with the same byte length and restores the pinned mtime, then proves the cached call still returns the *first* parse and `forceRefresh` returns the new one; separate tests cover an mtime change re-reading and `clearLocalGGUFModelCache()` emptying the cache.
- [x] 130. Test missing file → graceful `ok: false` — **DONE at both layers.** SDK: `resolveLocalGGUFModels` rejects with `GGUFParseError`/`ENOENT`. Host: `catalog.resolveModels` resolves to `{ ok: false, error: { kind: "config", code: "ENOENT" } }`.
- [x] 131. Test corrupted file → graceful `ok: false` with parse error detail — **DONE.** SDK: `NOT_GGUF` (plain-text file) and `TRUNCATED` (header cut short) both raise typed errors with the offending path in the message. Host: bad-magic file → `{ ok: false, error: { kind: "shape", code: "NOT_GGUF" } }`.
- [x] 132. Run `bun -F @cline/llms test` + `bun run types` — **DONE.** `bunx vitest run src/providers/gguf-models.test.ts src/providers/gguf-parser.test.ts` → **32 passed** (12 new + 20 existing, incl. the pre-existing parser suite, unbroken by the `GGUFMetadata` additions). `bun tsc -p sdk/packages/llms/tsconfig.dev.json --noEmit` → **exit 0**. Host side: `bun run build:sdk`, then `bunx vitest run src/sdk/model-catalog/` in `apps/vscode` → catalog/local-model-file/fingerprint/effective-config suites green (see gate note below).

> **Phase 6 decisions worth carrying forward**
> 1. `GGUFMetadata` gained `name`, `description`, `hasVisionEncoder`. The parser stays the single reader of raw `general.*` / `clip.*` keys; the catalog never re-parses the file.
> 2. `EffectiveProviderConfig` gained an optional `modelPath` (typed top-level field, not an `extras` key), read in `effective-config.ts` from `providers.json` and hashed into `computeConfigFingerprint` so a different file invalidates the model cache. There is no legacy StateManager key for it, so providers.json is the only source.
> 3. The local-file record is reported as `source: "host-adapter"` — the existing catalog source vocabulary already has a name for host-resolved lists, so no new enum value is needed.
> 4. Detection is capability/shape based (`modelPath` present AND the SDK declares no `modelsSourceUrl`), so a stray `modelPath` cannot hijack Ollama/LM Studio's live model lists, and a future file-backed provider needs no change in the catalog.
